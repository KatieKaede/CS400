...
public class BinarySearchTree<T extends Comparable<T>> implements SortedCollectionInterface<T> {
    ...
    protected void rotate(Node<T> child, Node<T> parent) throws IllegalArgumentException {
    	...
    	// Check if nodes are null or invalid
    	if (child == null || parent == null || child == parent) {
            throw new IllegalArgumentException("Nodes are invalid");
        }

        // Check if the child is on the left or the right side of the parent
        boolean isLeftChild = (parent.down[0] == child);
        boolean isRightChild = (parent.down[1] == child);

        // If neither are true, throw argument
        ...
        if (isLeftChild) {
        	// if left child, we perform a right rotation, the child will be moved above parent
        	// The right subtree will become the left
        	Node<T> childRightTree = child.down[1];
        	parent.down[0] = childRightTree;
        	if (childRightTree != null) {
        		childRightTree.up = parent;
        	}
        	child.down[1] = parent;
        } else {
        	// Left rotation version occurs when child is on the right
        	Node<T> childLeftTree = child.down[0];
        	parent.down[1] = childLeftTree;
        	if (childLeftTree != null) {
        		childLeftTree.up = parent;
        	}
        	child.down[0] = parent;
        }
        ...
        // Update references for child and parent
        parent.up = child;
